...
async function buttonPush(context) {
	...
	const oldShadeState = parseInt( await SmartState.getState( context, 'shadeState' ));
	...
	if (newShadeState!=oldShadeState) {
		...
		SmartState.putState( context, 'shadeState', newShadeState.toString() );
	}	
}
...
*/
	...
	page.section('shades', section => {
		section.deviceSetting('shade0').capabilities(['momentary'])
			.permissions('rx');
		section.deviceSetting('shade1').capabilities(['momentary'])
			.permissions('rx');
		section.deviceSetting('shade2').capabilities(['momentary'])
			.permissions('rx');
		section.deviceSetting('shade3').capabilities(['momentary'])
			.permissions('rx')
	});
})
...
.updated(async (context, updateData) => {
	console.log("ShadeControl - start install/update");
	
	// initialize state variable(s)
	// SmartState.putState( context.event.appId, 'shadeState', '0' );
	SmartState.putState( context, 'shadeState', '0' );
...
*/
	...
	console.log('shadeControl - end install/update')
})
